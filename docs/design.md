...
## 4. 구조 선택
본 시스템은 `Supervisor 패턴`을 사용한다. Supervisor가 전체 상태를 읽고 다음 Agent를 호출하며, 재탐색, 재작성, 검증 실패, 종료 조건을 일관되게 통제한다. `Dispatcher`는 사용하지 않는다. 추가로 Supervisor는 worker를 호출하기 전에 `report_blueprint`를 먼저 생성하고, 최종 비교 판단과 최종 보고서 문장은 `Supervisor Synthesis` 단계에서 직접 작성한다.
...
- worker는 최종 아웃풋 작성자가 아니며 evidence packet만 생성한다.
- Review는 감리자이며 저자가 아니다.

## 6. Task Decomposition

| Task | 설명 | 담당 Agent | 선행 Task |
|---|---|---|---|
| T0 | report blueprint 생성 | Supervisor Blueprint | - |
| T1 | 시장 환경 조사 | Market Research Agent | T0 |
| T2 | LG에너지솔루션 전략 분석 | LGES Analysis Agent | T0, T1 |
| T3 | CATL 전략 분석 | CATL Analysis Agent | T0, T1 |
| T4 | 정량 데이터 정리 | LGES / CATL Analysis Agent | T2, T3 |
| T5 | 스키마 검증 | Schema Validation (Middleware) | T2, T3, T4 |
| T6 | candidate evidence 생성 | Comparison Agent | T5 |
| T7 | 최종 비교 판단 및 문장 작성 | Supervisor Synthesis | T6 |
| T8 | Review 수행 | Review Agent | T7 |
| T9 | 보고서 조립 | Report Assembly (코드) | T8 |
| T10 | 제출 체크리스트 검수 | 코드 검증 | T9 |
...
- 역할: 비교 축, comparability precheck, worker task spec 정의
- Input: 전체 `AgentState`
- Output: `report_blueprint`
...
### Comparison Agent
- 역할: 동일 스키마를 통과한 두 기업 프로필을 기준으로 supervisor synthesis용 candidate evidence 생성
- Input: `market_context`, `lges_profile`, `catl_profile`
- Output: `comparison_input_spec`, `synthesis_claims`, `score_criteria`, `metric_comparison_rows`

### Supervisor Synthesis
- 역할: final judgment, executive summary, 선택된 비교표, reference-only 표, SWOT, score rationale 직접 작성
- Input: `report_blueprint`, `market_context`, `lges_profile`, `catl_profile`, `synthesis_claims`, `score_criteria`, `metric_comparison_rows`
- Output: `executive_summary`, `selected_comparison_rows`, `reference_only_rows`, `supervisor_swot`, `supervisor_score_rationales`, `final_judgment`

### Review Agent
- 역할: 편향, 근거성, 비교 일관성, 결론-근거 연결성을 검토하고 재작성 필요 여부 판단
- Input: `executive_summary`, `selected_comparison_rows`, `supervisor_swot`, `supervisor_score_rationales`, `final_judgment`, `report_spec`
