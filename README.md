...
flowchart TD
    ...
    PRE --> INIT["🔧 build_initial_state\nRetryBudget · AgentState 초기화"]
    INIT --> LOOP

    subgraph LOOP["🔁 run_once loop  ·  max 20 iterations  (graph.py)"]
        direction TB
        SUP["🧠 supervisor_agent\n AgentState 전체 검사 → 우선순위 라우팅"]

        subgraph BLUEPRINT["Layer 1 — Blueprint"]
            SBP["supervisor_blueprint\n비교 축 4개 고정 · worker contract 정의\n→ report_blueprint"]
        end

        subgraph WORKERS["Layer 2 — Worker Evidence  (RAG)"]
            MR["market_research\nmarket fact extraction\n→ market_facts · market_context"]
            LA["lges_analysis\nLGES fact + normalization\n→ lges_facts · lges_profile · lges_normalized_metrics"]
            CA["catl_analysis\nCATL fact + normalization\n→ catl_facts · catl_profile · catl_normalized_metrics"]
        end

        subgraph COMP["Layer 3 — Comparison Candidate"]
            CMP["comparison\ncandidate evidence packet 생성\n→ synthesis_claims · score_criteria · metric_comparison_rows"]
        end

        subgraph SYNTH["Layer 4 — Supervisor-Owned Report"]
            SS["supervisor_synthesis\n최종 표 · SWOT · score rationale\n· executive summary · final_judgment 작성"]
        end

        subgraph REVIEW["Layer 5 — Review / Validation"]
            REV["review\n근거 정합성 · 비교 축 일관성 · 필수 섹션 감리\n→ review_result"]
            RESET["REVIEW_INVALIDATIONS\nrevision_target downstream fields 선택적 초기화\nschema_retry_count 리셋"]
        end

        SUP -->|"① report_blueprint 없음"| SBP
        SUP -->|"② market_context 없음"| MR
        SUP -->|"③ lges_profile 없음"| LA
        SUP -->|"④ catl_profile 없음"| CA
        SUP -->|"⑤ synthesis_claims 없음"| CMP
        SUP -->|"⑥ supervisor sections 없음"| SS
        SUP -->|"⑦ review_result 없음"| REV
        ...
        CMP --> SUP
        SS --> SUP
        REV -->|"review_result.passed = False\n+ retry 예산 남음"| RESET
        RESET --> SUP
    end

    SUP -->|"⑧ 전부 충족 → finish"| FIN
...
flowchart LR
    ...
    subgraph PRE["전처리  (tools.preprocessing / retrieval)"]
        CHUNK["청킹\n1200자 · overlap 200"]
    ...
    subgraph BLUEPRINT_A["① Supervisor Blueprint"]
        SBP_A["supervisor_blueprint_agent\n비교 축 4개 고정\nworker contract 정의\ncomparability precheck"]
        BP_OUT["report_blueprint\nReportBlueprint\n(model_validator 강제)"]
        SBP_A --> BP_OUT
    end

    subgraph MKT["② Market Research  (worker)"]
        MR_A["market_research_agent\nscope=market retrieval\nfact extraction"]
        MR_OUT["market_facts\nmarket_context\nmarket_context_summary"]
        MR_A --> MR_OUT
    end

    subgraph LGES_BOX["③ LGES Analysis  (worker)"]
        LA_A["lges_analysis_agent\nscope=lges retrieval\nfact + normalization"]
        LA_OUT["lges_facts\nlges_profile\nlges_normalized_metrics\nprofitability_reported_rows"]
        LA_A --> LA_OUT
    end

    subgraph CATL_BOX["④ CATL Analysis  (worker)"]
        CA_A["catl_analysis_agent\nscope=catl retrieval\nfact + normalization"]
        CA_OUT["catl_facts\ncatl_profile\ncatl_normalized_metrics\nprofitability_reported_rows"]
        CA_A --> CA_OUT
    end

    subgraph CMP_BOX["⑤ Comparison  (worker)"]
        CSPEC["tools.comparison_contract\nfact layer →\nComparisonInputSpec\n각 사 최대 12 claim"]
        CMP_A["comparison_agent\n새 retrieval 없음\nclaim catalog만 사용"]
        CMP_OUT["synthesis_claims\nscore_criteria\nmetric_comparison_rows\nlow_confidence_claims"]
        CSPEC --> CMP_A --> CMP_OUT
    end

    subgraph SS_BOX["⑥ Supervisor Synthesis"]
        SS_A["supervisor_synthesis_agent\n최종 보고서 섹션 직접 작성"]
        SS_OUT["selected_comparison_rows\nreference_only_rows\nexecutive_summary\nsupervisor_swot\nfinal_judgment\nimplications · limitations"]
        SS_A --> SS_OUT
    end

    subgraph REV_BOX["⑦ Review"]
        RSPEC["tools.reporting\nbuild_report_spec\n→ ReportSpec"]
        REV_A["review_agent\n근거 정합성 · 비교 축 감리"]
        REV_OUT["review_result\n(passed / revision_target)"]
        RSPEC --> REV_A --> REV_OUT
    end

    subgraph EXPORT_BOX["⑧ Report Assembly"]
        VGATE_A{"hard gate\nvalidate_final_\ndelivery_state"}
        MD["report.md"]
        HTML["report.html"]
        PDF_OUT["report.pdf\n(Playwright)"]
        VGATE_A -->|"통과"| MD & HTML & PDF_OUT
    end

    ENV --> SBP_A
    FAISS --> MR_A & LA_A & CA_A
    BP_OUT --> MR_A & LA_A & CA_A
    MR_OUT & LA_OUT & CA_OUT --> CSPEC
    MR_OUT & LA_OUT & CA_OUT & CMP_OUT --> SS_A
    BP_OUT --> SS_A
    SS_OUT --> RSPEC
    REV_OUT -->|"passed"| VGATE_A
    REV_OUT -->|"failed"| RESET_A["REVIEW_INVALIDATIONS\ndownstream 초기화\n→ supervisor 재라우팅"]
```
...
LLM 응답이 들어온 뒤 Pydantic `model_validate()`를 거쳐 AgentState에 쓰이기까지의 검증 경로입니다. "prompt + schema + validator + retry"가 함께 동작하는 구조의 실체를 보여줍니다.

```mermaid
flowchart TD
    LLM["🤖 LLM 응답 수신\n(raw JSON)"]

    LLM --> MV["Pydantic model_validate()"]

    MV --> V1{"ReportBlueprint\nmodel_validator"}
    V1 -->|"① comparison_axes 4개 순서 불일치\n② worker_task_specs 3개 미만\n③ forbidden_outputs 누락"| FAIL_BP["ValueError\nschema retry →\nsupervisor_blueprint 재시도"]
    V1 -->|"통과"| BP_OK["report_blueprint\n✅ AgentState 업데이트"]

    MV --> V2{"FactExtractionOutput\nmodel_validator"}
    V2 --> V2A{"scope alignment\n검사"}
    V2A -->|"claim.scope ≠ extraction.scope"| FAIL_SCOPE["ValueError\n'LGES claim이 CATL 컨텍스트에 있음'\nschema retry"]

    V2A -->|"통과"| V2B{"required metric\nfamilies 검사"}
    V2B -->|"LGES: revenue_growth_guidance\noperating_margin / capex\ness_capacity / secured_order_volume\n중 하나라도 없음"| FAIL_MF["ValueError\nschema retry →\nlges_analysis 재시도"]
    V2B -->|"CATL: revenue / gross_profit_margin\nnet_profit_margin / roe\noperating_cash_flow 중 하나 없음\n(revenue+profit_for_year 있으면 margin 면제)"| FAIL_MFC["ValueError\nschema retry →\ncatl_analysis 재시도"]
    V2B -->|"통과"| FACT_OK["lges_facts / catl_facts\n✅ AgentState 업데이트"]

    MV --> V3{"ComparisonEvidenceOutput\nvalidation\n(tools/validation.py)"}
    V3 -->|"SynthesisClaim\nsupporting_claim_ids < 2"| FAIL_SYN["hard-gate:\nsynthesis-support-count"]
    V3 -->|"supporting_claim_ids가\nallowed_claim_ids 범위 밖"| FAIL_ORIG["hard-gate:\nsupporting-claim-origin"]
    V3 -->|"ScoreCriterion\nevidence_refs 없음"| FAIL_SCORE["hard-gate:\nscore-criterion-evidence"]
    V3 -->|"통과"| CMP_OK["comparison_input_spec\nsynthesis_claims\n✅ AgentState 업데이트"]

    MV --> V4{"ReportSpec\nmodel_validator"}
    V4 -->|"claim_id 중복 발견"| FAIL_DUP["ValueError\n'ReportSpec claim_id must be unique'\nschema retry"]
    V4 -->|"통과"| SPEC_OK["report_spec\n✅ 제출 계약 객체 완성"]

    subgraph RETRY["Supervisor Schema Retry Logic"]
        SR{"schema_retry_count\n< schema_validation_max?"}
        SR -->|"Yes  (기본 한도: 2)"| REDO["현재 step 재시도\nschema_retry_count + 1"]
        SR -->|"No  예산 소진"| ABORT["finish  status=failed"]
    end

    FAIL_BP & FAIL_SCOPE & FAIL_MF & FAIL_MFC & FAIL_DUP --> SR
```
...
아래 다이어그램은 문서 원문에서 최종 판단까지 근거가 어떻게 연결되는지 전체 lineage를 보여줍니다.

```mermaid
flowchart TD
    subgraph SOURCE["원문 소스"]
        DOC["📄 DocumentRef\ndocument_id · title\nsource_path · published_at · page_range"]
    end

    subgraph RETRIEVAL["RAG Retrieval"]
        CHUNK2["ProcessedChunk\nchunk_id · document_id · page · text"]
        EREF["EvidenceRef\ndocument_id · chunk_id\npage · snippet · score"]
        DOC --> CHUNK2 --> EREF
    end

    subgraph FACT["Fact Layer  (worker agents)"]
        direction TB
        AFC["AtomicFactClaim\nclaim_id = {scope}-{category}-{ordinal}\nclaim_text\nevidence_refs ← EvidenceRef 1+개 필수"]
        MFC["MetricFactClaim\nclaim_id = {scope}-{category}-{ordinal}\nmetric_name · value · unit · period\nreported_basis  (guidance / reported)\nevidence_refs ← EvidenceRef 1+개 필수"]
        EREF --> AFC
        EREF --> MFC
    end

    subgraph NORM["Normalization  (tools.normalization)"]
        NM["NormalizedMetric\nnormalized_metric_name\nbasis · period · unit 보존\nis_derived = True  (파생 지표인 경우)\nsource_claim_ids · evidence_refs"]
        MCR["MetricComparisonRow\nrow_id · metric_name · period\nlges_value · catl_value · basis_note\ncomparability_status\n(direct / reference_only / reject)"]
        MFC --> NM --> MCR
    end

    subgraph COMPARISON["Comparison Layer  (comparison agent)"]
        CIS["ComparisonInputSpec\nlges_catalog  최대 12 claim\ncatl_catalog  최대 12 claim\nallowed_claim_ids() ← 이 범위만 허용"]
        SC["SynthesisClaim\nclaim_id · claim_text\nsupporting_claim_ids  2개 이상 필수\n※ allowed_claim_ids 범위 강제"]
        SCRI["ScoreCriterion\ncriterion_key · company_scope · score 1-5\nrationale\nsupporting_claim_ids\nevidence_refs  ← materialized 필수"]
        AFC & MFC --> CIS
        CIS --> SC
        CIS --> SCRI
    end

    subgraph SUPERVISOR["Supervisor-Owned Layer  (supervisor_synthesis)"]
        FJ["FinalJudgment\njudgment_text\nsupporting_claim_ids\nconfidence_level"]
        SSR["supervisor_score_rationales\nScoreCriterion 목록\nevidence_refs  materialized"]
    ...
    subgraph REPORT["ReportSpec  (제출 계약)"]
        RS["ReportSpec\natom_claims · metric_claims · synthesis_claims\nswot_matrix · score_criteria\nmetric_comparison_rows · charts\nfinal_judgment · references\n※ claim_id 중복 시 model_validator ValueError"]
        FJ & SSR & SWOT & MCR --> RS
    end

    subgraph TRACE["역추적 경로"]
        direction LR
        T1["FinalJudgment"] -->|"supporting_claim_ids"| T2["SynthesisClaim"]
        T2 -->|"supporting_claim_ids"| T3["AtomicFactClaim\nMetricFactClaim"]
        T3 -->|"evidence_refs"| T4["EvidenceRef"]
        T4 -->|"document_id · page"| T5["DocumentRef\n원문 페이지"]
    end
```
...
현재 기본 워크플로우에 HITL은 없습니다. 확장한다면 `comparison` 직후 · `supervisor_synthesis` 직전이 가장 적절한 삽입 위치입니다.

- **입력**: worker fact packet, normalized metrics, comparison candidate evidence
- **사람이 할 일**: 누락 키워드 추가, 재검색 요청, 비교 축 보정
- **하면 안 되는 일**: 최종 보고서 직전에서 결론을 직접 수정하거나 제출물을 수동으로 덮어쓰기

HITL을 추가하더라도 최종 output ownership은 Supervisor 쪽에 그대로 남아야 합니다.
