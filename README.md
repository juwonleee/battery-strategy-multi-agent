...
본 프로젝트는 Supervisor가 전체 상태를 읽고 다음 Agent를 선택하는 중앙 제어형 워크플로우입니다. 핵심은 Supervisor가 먼저 `report_blueprint`를 만들고, worker는 evidence packet만 생성하며, 최종 비교 판단과 최종 보고서 문장은 다시 Supervisor가 직접 작성한다는 점입니다.

```mermaid
flowchart TD
    A["Supervisor Blueprint"] --> B["Market Research Agent"]
    A --> C["LGES Analysis Agent"]
    A --> D["CATL Analysis Agent"]
    B --> E["Comparison Evidence Agent"]
    C --> E
    D --> E
    E --> F["Supervisor Synthesis"]
    F --> G["Review Agent"]
    G --> H["Report Assembly"]
```
...
1. `Supervisor Blueprint` 단계가 비교 축, comparability precheck, worker task spec을 확정합니다.
2. `Market Research Agent`가 시장 배경과 비교 축 힌트를 추출합니다.
3. `LGES Analysis Agent`가 LGES의 전략, 핵심 주장, 정량 근거를 추출합니다.
4. `CATL Analysis Agent`가 CATL의 전략, 핵심 주장, 정량 근거를 추출합니다.
5. 정량 claim은 normalization을 거쳐 비교 가능한 형태로 정리됩니다.
6. `Comparison Agent`는 candidate evidence packet만 생성하고, final judgment를 작성하지 않습니다.
7. `Supervisor Synthesis`가 표/차트 채택, SWOT, score rationale, executive summary, final judgment를 직접 작성합니다.
8. `Review Agent`가 근거 연결, 점수 근거, 결론 일관성, 레이아웃 필수 요소를 감리합니다.
9. 최종 상태가 hard gate를 통과하면 Markdown, HTML, PDF 보고서와 실행 로그를 생성합니다.
...
| Comparison Pass | 1차 pass에서 만든 claim catalog만 사용해 candidate evidence 생성 |
...
| Agent | 역할 | 주요 출력 |
|------|------|----------|
| Supervisor Blueprint | 비교 축, comparability, worker contract 정의 | `report_blueprint` |
...
| Comparison Agent | supervisor synthesis용 candidate evidence 생성 | `synthesis_claims`, `score_criteria`, `metric_comparison_rows` |
| Supervisor Synthesis | 최종 비교 판단, 표/차트 선택, 최종 문장 작성 | `executive_summary`, `selected_comparison_rows`, `supervisor_swot`, `supervisor_score_rationales`, `final_judgment` |
| Review Agent | 결론-근거 정합성과 제출 가능성 감리 | `review_result`, `review_issues`, `validation_warnings` |
